package com.skuniv.modoo.member.controller;

import com.skuniv.modoo.dto.MemberRequestDto;
import com.skuniv.modoo.member.dao.TokenDao;
import com.skuniv.modoo.member.vo.SigninVO;
import com.skuniv.modoo.service.AuthService;
...
public class MemberController {
     ...
     * <p>
     * 입력받은 이메일과 비밀번호를 검증하고, 성공적인 인증 후 JWT 토큰을 {@link TokenDao} 형태로 반환합니다.
     * </p>
     * <br>
     * @param signinVO 로그인 요청 정보를 담은 객체입니다. 이메일과 비밀번호를 포함합니다.
     * @return 로그인 성공 시 JWT 토큰을 담은 {@link TokenDao} 객체를 {@link ResponseEntity}에 담아 반환합니다.
    ...
    public ResponseEntity<Object> login (@RequestBody SigninVO signinVO) {
        ...
        // TODO 아래 리턴값 수정해야함.
        // 사용자 로그인 처리
        TokenDao tokenDao = authService.login(new MemberRequestDto(email, password));

        return ResponseEntity.ok(tokenDao);
    }
}
